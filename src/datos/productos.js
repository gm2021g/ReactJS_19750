/*export const productos = [
    {
        id: 1,
        nombre: "Antiparasitario Interno Perros Y Gatos",
        tipo: "medicamentos",
        marca: "Total Full",
        descripcion: "Antiparasitario Interno Perros Y Gatos - 15 Cc",
        precio: 490,
        imagen: "../images/382117.jpg",
        stock: 20
    },
    {
        id: 2,
        nombre: "Curabichera",
        tipo: "medicamentos",
        marca: "Bactrovet",
        descripcion: "Curabichera Plata AM Pet 250cc",
        precio: 385,
        imagen: "../images/338018.jpg",
        stock: 10
    },
    {
        id: 3,
        nombre: "Antiparasitario Perro/Gato",
        tipo: "medicamentos",
        marca: "Meltra",
        descripcion: "Antiparasitario Perro/Gato Suspens. Cachorro 15 Ml",
        precio: 950,
        imagen: "../images/361043.jpg",
        stock: 5
    },
    {
        id: 4,
        nombre: "Antiparasitario Gatos",
        tipo: "medicamentos",
        marca: "Feline",
        descripcion: "Antiparasitario Endospot",
        precio: 800,
        imagen: "../images/354252.jpg",
        stock: 20
    },
    {
        id: 5,
        nombre: "Pipeta Perros",
        tipo: "medicamentos",
        marca: "Frontline Plus",
        descripcion: "Pipeta Perro 2-10 kg",
        precio: 855,
        imagen: "../images/373011.png",
        stock: 15
    },
    {
        id: 6,
        nombre: "Pipeta Perro",
        tipo: "medicamentos",
        marca: "Power",
        descripcion: "Pipeta Perro 2-4 kg",
        precio: 295,
        imagen: "../images/302220.jpg",
        stock: 5
    },
    {
        id: 7,
        nombre: "Collar Antipulgas Gato",
        tipo: "medicamentos",
        marca: "Ecthol",
        descripcion: "Collar Antipulgas Para Gato",
        precio: 740,
        imagen: "../images/332003.png",
        stock: 10
    },
    {
        id: 8,
        nombre: "Vitaminico Mineral Aves",
        tipo: "medicamentos",
        marca: "Porta",
        descripcion: "Bloque Vitaminico Mineral para Aves",
        precio: 300,
        imagen: "../images/135012.jpg",
        stock: 6
    },
    {
        id: 9,
        nombre: "Protector contra Piojos Aves",
        tipo: "medicamentos",
        marca: "Porta",
        descripcion: "Protector contra Piojos para Aves - 12gr",
        precio: 1500,
        imagen: "../images/135013.jpg",
        stock: 8
    },
    {
        id: 10,
        nombre: "Collar contra Pulgas y Garrapatas",
        tipo: "medicamentos",
        marca: "Seresto",
        descripcion: "Collar contra Pulgas y Garrapatas Perros y Gatos Hasta 8KG",
        precio: 3470,
        imagen: "../images/132000.jpg",
        stock: 20
    },
    {
        id: 11,
        nombre: "Limpia oidos",
        tipo: "medicamentos",
        marca: "Porvet",
        descripcion: "Limpia oidos Perros y Gatos 100cc",
        precio: 570,
        imagen: "../images/235114.png",
        stock: 12

    },
    {
        id: 12,
        nombre: "Gotas para ojos antibacterial",
        tipo: "medicamentos",
        marca: "Lab Love Sudamericana",
        descripcion: "Antibacterial Perro con Esteroides 10 Cc",
        precio: 805,
        imagen: "../images/352012.jpg",
        stock: 10
    },
    {
        id: 13,
        nombre: "Shampoo Pulguicida Garrapaticida Perros y Gatos",
        tipo: "higiene",
        marca: "Osspret",
        descripcion: "Shampoo Pulguicida Garrapaticida Tradicional Perros y Gatos 250 Cc",
        precio: 780,
        imagen: "../images/1254233.jpg",
        stock: 8
    },
    {
        id: 14,
        nombre: "Espuma Seca En Aerosol Perros y Gatos",
        tipo: "higiene",
        marca: "Paul",
        descripcion: "Espuma Seca En Aerosol 250 Cc",
        precio: 1220,
        imagen: "../images/335030.jpg",
        stock: 20
    },
    {
        id: 15,
        nombre: "Shampoo Perro Porta Cachorro Hipoalergenico",
        tipo: "higiene",
        marca: "Lab Love Sudamericana",
        descripcion: "Shampoo Perro Porta Cachorro Hipoalergenico 500 Ml",
        precio: 805,
        imagen: "../images/335159.jpg",
        stock: 40
    },
    {
        id: 16,
        nombre: "Shampoo 2 En 1 Antifrizz Brillo Y Docilidad Perro",
        tipo: "higiene",
        marca: "MasKota",
        descripcion: "Shampoo 2 En 1 Antifrizz Brillo Y Docilidad Perro 300 Ml",
        precio: 650,
        imagen: "../images/240260.png",
        stock: 10
    },
    {
        id: 17,
        nombre: "Shampoo Bactericida con Clorhexidina",
        tipo: "higiene",
        marca: "Osspret",
        descripcion: "Shampoo Bactericida con Clorhexidina 250 cc",
        precio: 760,
        imagen: "../images/366192.jpg",
        stock: 20
    },
    {
        id: 18,
        nombre: "Shampoo Antialergico Caninos Y Felinos",
        tipo: "higiene",
        marca: "Moksha",
        descripcion: "Shampoo Antialergico Caninos Y Felinos 250 Cc",
        precio: 675,
        imagen: "../images/371045.jpg",
        stock: 15
    },
    {
        id: 19,
        nombre: "Shampoo Analergenico Para Cachorros",
        tipo: "higiene",
        marca: "Moksha",
        descripcion: "Shampoo Analergenico Para Cachorros 250 Cc",
        precio: 480,
        imagen: "../images/371044.jpg",
        stock: 2
    },
    {
        id: 20,
        nombre: "Shampoo Antimicótico",
        tipo: "higiene",
        marca: "Osspret",
        descripcion: "Shampoo Antimicótico 250 Cc",
        precio: 1120,
        imagen: "../images/4554344.jpg",
        stock: 9
    },
    {
        id: 21,
        nombre: "Moises Classic",
        tipo: "accesorios",
        marca: "Wepets",
        descripcion: "Moises Classic Grafito Medio",
        precio: 4500,
        imagen: "../images/244104.png",
        stock: 3
    },
    {
        id: 22,
        nombre: "Cama redonda",
        tipo: "accesorios",
        marca: "Cocooning",
        descripcion: "Cama Emir Redonda Marron Chocolate Talle Único",
        precio: 5300,
        imagen: "../images/248039.jpg",
        stock: 15
    },
    {
        id: 23,
        nombre: "Moises Redondo",
        tipo: "accesorios",
        marca: "Wepets",
        descripcion: "Moises Redondo Tropical Talle Único",
        precio: 6540,
        imagen: "../images/422331.jpg",
        stock: 20
    },
    {
        id: 24,
        nombre: "Comedero Inoxidable Doble",
        tipo: "accesorios",
        marca: "Lazy Dog",
        descripcion: "Comedero Inoxidable Doble Kitty Único",
        precio: 1320,
        imagen: "../images/239275.jpg",
        stock: 7
    },
    {
        id: 25,
        nombre: "Bebedero Automático",
        tipo: "accesorios",
        marca: "Stefanplast",
        descripcion: "Bebedero Automático Break Pequeño",
        precio: 2170,
        imagen: "../images/766587.jpg",
        stock: 5
    },
    {
        id: 26,
        nombre: "Dispenser Automatico Perros Y Gatos",
        tipo: "accesorios",
        marca: "Azimut",
        descripcion: "Dispenser Automatico Para Perros Y Gatos - 0,6 L",
        precio: 980,
        imagen: "../images/249170-01.jpg",
        stock: 3
    },
    {
        id: 27,
        nombre: "Litera Para Hamsters",
        tipo: "accesorios",
        marca: "Pawise",
        descripcion: "Litera Para Hamsters Mediano Chico",
        precio: 1290,
        imagen: "../images/237353.png",
        stock: 6
    },
    {
        id: 28,
        nombre: "Suplemento Flourish Potassium para pecera",
        tipo: "accesorios",
        marca: "Seachem",
        descripcion: "Suplemento Flourish Potassium para pecera 100Ml",
        precio: 1120,
        imagen: "../images/227011.jpg",
        stock: 12
    },
    {
        id: 29,
        nombre: "Bebedero para medicamentos Aves",
        tipo: "accesorios",
        marca: "Millex",
        descripcion: "Bebedero para medicamentos Aves Unico",
        precio: 100,
        imagen: "../images/238125.jpg",
        stock: 2
    },
    {
        id: 30,
        nombre: "Porta Lamparas Para Terrarios",
        tipo: "accesorios",
        marca: "Exo Terra",
        descripcion: "Porta Lamparas Compact Top Para Terrarios XL",
        precio: 1316,
        imagen: "../images/253005-01.jpg",
        stock: 6
    }
]

*/